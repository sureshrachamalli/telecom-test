...
public interface RewardsAndPaymentsRepository extends JpaRepository<RewardsAndPaymentsEntity, Long> {
   ...
   @Query(value = "SELECT * FROM PAYMENTS_AND_REWARDS p where p.USER_ID = :userId", nativeQuery = true)
    ...
    @Query(value = "DELETE FROM PAYMENTS_AND_REWARDS p where p.CREATED_DATE < :fromDate", nativeQuery = true)
